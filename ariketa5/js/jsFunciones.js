...
function sel_id() {
    ...
    //  console.log(idJson);
    $.ajax({
		//url: "./php/getIdPrepare/",
        url: "./php/getIdNormal/",
        type: "GET",
        data: { param: numIdBuscar}, //aldagai bakarra pasatzeko get, asko bidaltzeko post bidez eta idJson barruan atalak definituta
        //data: { param: JSON.stringify(idJson) },
        dataType: 'json',
        success: function(response) {
            // alert (response.status);
            // confirm ("Seguru ezabatu nahi duzula " + numIdBuscar + " id erregistroa?");
			console.log(response);
			var myHtml = "";
			document.getElementById("divMain").innerHTML = myHtml;
			if(response.length==0){
