...
if (!($conn->connect_error)) {
    ...
    //$query = "SELECT * FROM users WHERE id = ? AND company = ?";   // $query is a simple variable (string)
    $query = "SELECT * FROM users WHERE ";
    if ($params->id == ""){
      $query .= "company = ?";
      $stmt = $conn->prepare($query);
      $stmt->bind_param('s', $params->empresa);
    } else if ($params->empresa == "") {
      $query .= "id = ?";
      $stmt = $conn->prepare($query);
      $stmt->bind_param('i', $params->id);
    } else {
      $query .= "id = ? AND company = ?";
      $stmt = $conn->prepare($query);
      $stmt->bind_param('is', $params->id, $params->empresa);
    }
       // prepare statement
